 ...
 * Set class and other attribut (excluding name) for the element on the $attr parameter eg. $attr = 'id="fancythemes-multiple-1" class="fancythemes-multiple"' 
 *
 *
 * @since fancythemes 1.0
 */
function fancythemes_post_multiple_select( $id, $args = 'post_type=post&post_status=publish', $default, $attr = ''){
 ...
 * @since fancythemes 1.0
 */
function fancythemes_post_select( $id, $args = 'post_type=post&post_status=publish', $default, $attr = ''){
	$posts = get_post($args);
	if ( is_array($posts) && count($posts)> 0 ){
	?>
 ...
 * @since fancythemes 1.0
 */
function fancythemes_term_select( $id, $taxonomy = 'category', $args = 'hide_empty=1', $default, $attr = ''){
 ...
 * @since fancythemes 1.0
 */
function fancythemes_term_multiple_select( $id, $taxonomy='category', $args = 'hide_empty=1', $default, $attr = ''){
	$terms = get_terms($taxonomy, $args);
	if ( is_array($terms) && count($terms)> 0 ){
	?>
		<select multiple="multiple" name="<?php echo $id; ?>[]" <?php echo $attr; ?> >
